...
try:
    ...
    while True:
        ...
        frame = vs.read() ## taking directly a cv2 object is faster?
        ...
        for face in faces: # here in theory we should "iterate" only on a single face
            ...
            right_blink_ratio = get_blinking_ratio(right_eye_pts, landmarks) # gestures.
            left_blink_ratio = get_blinking_ratio(left_eye_pts, landmarks) # gestures.
            avg_blink_ratio = (left_blink_ratio + right_blink_ratio) / 2

            # detect gaze ratios
            right_gaze_ratio = get_gaze_ratio(right_eye_pts, landmarks) # gestures.
            left_gaze_ratio = get_gaze_ratio(left_eye_pts, landmarks) # gestures.
        ...
        if conf['show_video']:
            ...
            ### ord() returns an integer representing the Unicode code point of that character.  ord('a') returns the integer 97 and ord('€') (Euro sign) returns 8364
